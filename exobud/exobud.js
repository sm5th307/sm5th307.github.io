...
function initExobud(){
 ...
 if(blnStatusBar){ window.status=('ExoBUD MP(II) v4.1tc+ 網站媒體播放程式');}
...
function wmpStop(){
 ...
 if(blnStatusBar){ window.status=('ExoBUD MP(II) v4.1tc+ 網站媒體播放程式 [就緒]');return true;}
}
...
function chkRept(){
 ...
 if(blnRept){
   blnRept=false; imgChange('rept',0); disp2.innerHTML="不重複播放";
 } else {
   blnRept=true; imgChange('rept',1); disp2.innerHTML="重複播放";
 }
...
function evtWmpError(){
 ...
 if(intErrCnt<=3){
   disp2.innerHTML = "嘗試連線 (" + intErrCnt + ")";
